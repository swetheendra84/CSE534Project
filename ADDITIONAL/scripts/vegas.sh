...
while IFS='' read -r line || [[ -n "$line" ]]; do
    ...
    ssh -p 130 -n $user@$host "echo vegas > /proc/sys/net/ipv4/tcp_congestion_control && hping3 -S -V -c 5 -p 80 ${array[0]} > ${array[0]}.txt 2>&1 && grep 'round-trip' ${array[0]}.txt"
